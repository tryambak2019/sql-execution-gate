...
from .bigquery_agent import bq_root_agent
...
def log_conversation_to_file(callback_context: CallbackContext) -> None:
    """Appends each agent turn to a conversation log file with actual UI messages."""
    try:
        log_file = Path("scratchwork/logs/conversation_logs.jsonl")
        log_file.parent.mkdir(parents=True, exist_ok=True)
        
        # Access the session events which contain all conversation messages
        events = callback_context._invocation_context.session.events
        
        # Extract the last few events to get user message and agent response
        recent_messages = []
        ...
        logging.getLogger(__name__).warning(f"Failed to log conversation: {e}")
...
interactive_planner_agent = LlmAgent(
    ...
    **DATA QUERIES:** If the user request involves data analysis, database queries, or BigQuery-related tasks, delegate directly to the `bq_root_agent` sub-agent instead of creating a research plan.
    ...
    """,
    sub_agents=[research_pipeline, bq_root_agent],
    tools=[AgentTool(plan_generator)],
    generate_content_config=genai_types.GenerateContentConfig(temperature=0.01, automatic_function_calling=genai_types.AutomaticFunctionCallingConfig(disable=True)),
    output_key="research_plan",
    after_agent_callback=log_conversation_to_file,
)
